...
Deriva de `docs/consignas/*.pdf`, `.claude/context/01-producto-y-consignas.md` y las
...
| BE-SEC-06 | Autorización por alcance del `SELLER` | POST-E1 | Un `SELLER` sólo lista/edita empresas, contactos y oportunidades asignadas a él (ver DP-02 — pendiente de resolución exacta) |
...
| ID | Requisito | Prioridad | Criterio de aceptación |
|---|---|---|---|
| BE-CUS-01 | Alta de empresa | E1 | `POST /api/v1/companies` con los datos mínimos de `01-producto-y-consignas.md`; `name` y `status` obligatorios; `cuit` único por tenant si está presente |
...
| BE-OFF-02 | ABM completo de salones | EF | `POST`/`PUT`/`GET` de `Venue`; `capacity` y `active` editables sólo por rol habilitado (DP-04) |
...
| ID | Requisito | Prioridad | Criterio de aceptación |
|---|---|---|---|
| BE-OPP-01 | Alta de oportunidad | E1 | `POST /api/v1/opportunities`: `title`, (`companyId` **o** `contactId`, al menos uno), `salesRepId`, `venueId`, etapa inicial, `eventDate`, `attendeeCount` |
...
| BE-OPP-10 | Validación de capacidad del salón | EF | `attendeeCount` no puede superar `venue.capacity`; violación → `422` (ver DP-08 para el nicho de 50 personas) |
| BE-OPP-11 | Reserva sin solapamiento | EF | Una oportunidad `GANADA` no puede compartir salón y rango de fechas con otra `GANADA` del mismo tenant; violación → `409`. Restricción de aplicación **y** restricción GiST en PostgreSQL (ver DP-07 para semántica exacta del rango) |
...
Un requisito marcado como bloqueado por un DP no se implementa "a medias": se implementa
bajo el supuesto declarado en `docs/decisiones/decisiones-pendientes.md` y se ajusta con
una migración nueva si la resolución final difiere.
