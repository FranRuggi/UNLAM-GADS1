...
`CLAUDE.md` / `.claude/context/`. No repite lo ya acordado ahí; lo referencia.
...
Se generan **al finalizar la implementación** (última fase de `tasks.md`), salvo las
anotaciones Swagger, que se agregan controller por controller a medida que se crean —
no se posponen. Todo vive en `backend/docs/`, salvo `backend/README.md`.
...
```powershell
./mvnw spring-boot:run  # en otra terminal
curl http://localhost:8080/v3/api-docs.yaml -o backend/docs/openapi.yaml
```
...
4. Variables de entorno: tabla con nombre, ejemplo y obligatoriedad, más referencia a
   `.env.example`.
5. Cómo levantar en local sin Docker (`./mvnw spring-boot:run`, perfil `local`).
...
11. Estructura de paquetes (resumen breve, con link a `.claude/context/02-backend-convenciones.md`
    para el detalle).
...
- **De punta a punta (`*IT`)**: un test por flujo relevante, entrando por HTTP real
  ...
  silencio durante la Fase 2 — ver `.claude/specs-backend/tasks.md`).
